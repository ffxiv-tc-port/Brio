...
public static class NativeHelpers
{
    ...
    /// 🔴 <b>兩個值必定不相等。</b> 位移是 <c>alignment - (base % alignment)</c>,值域 <c>1..alignment</c> —— 基底本來就對齊時算出來的是一整個 <c>alignment</c>,<b>不是 0</b>。
    /// ⇒ <b>釋放一定要用 <see cref="FreeAlignedMemory"/>(它釋放 Unaligned)</b>; 拿 <c>Aligned</c> 去 <see cref="FreeMemory"/> 是對堆積區塊中間呼叫 <c>LocalFree</c>,結果是堆積損壞,而且當場不報錯。
    /// </summary>
